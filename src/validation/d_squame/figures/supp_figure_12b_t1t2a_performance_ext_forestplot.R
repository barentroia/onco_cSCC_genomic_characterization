#-------------------------------------------------------------------------------
# Aim: Plot of SCCore-GEP performance in T1-T2a subset of D-SQUAME validation
# Author: L.Pozza
# Input: SCCore-GEP performances
# Output: Supplementary Figure 10B: forest plot of SCCore-GEP T1-T2a D-SQUAME validation performances
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
df <- res %>%
    filter(model %in% c("AJCC8_model", "BWH_model", "CP_risk", "sccore_gep") & variable %in% c("BWH_staging", "Type_of_material_bin")) %>%
    ...
    mutate(wcindex_pe = as.numeric(sapply(strsplit(wcindex, "[(|)|-]"), "[[", 1)),
           ...
           group = recode(group, "Excision/Reexcision/Resectie/Mohs" = "Excision"),
           variable = recode(variable, "BWH_staging" = "BWH", "Type_of_material_bin" = "Sample type"),
...
fp_df <- df %>%
    ...
    rbind(data.frame(group_model = c("Sample type",
                                     "Biopsy", "Excision",
                                     "BWH",
                                     "T1", "T2a"),
                     category = c("header",
    ...
    mutate(group_model = factor(group_model,
                                levels = c("BWH:T2a:EMC model", "BWH:T2a:SCCore-GEP",
                                           "T2a",
...
# Save results
#-------------------------------------------------------------------------------
write.csv(source_data_df, file.path(output_dir, "supp_figure_12b_t1t2a_performance_ext_forestplot_source_data.csv"), row.names = F)
pdf(file.path(output_dir, "supp_figure_12b_t1t2a_performance_ext_forestplot.pdf"),
    width = 8.5, height = 6)
print(fp)
dev.off()
#-------------------------------------------------------------------------------