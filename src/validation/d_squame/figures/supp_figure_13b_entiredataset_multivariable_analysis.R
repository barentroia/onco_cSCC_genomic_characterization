#-------------------------------------------------------------------------------
# Aim: Forest plot of multivariable analysis in the entire D-SQUAME validation dataset
# Author: L.Pozza
# Input: Multivariable analysis results of SCCore-GEP vs BWH, AJCC8 and EMC model
# Output: Supplememntary Figure 13B: forest plot of multivariable analysis in entire D-SQUAME validation dataset
#-------------------------------------------------------------------------------
...
sccoregep_bwh_fn <- file.path(results_dir, "intermediate", "validation", "d_squame", "multivariable_analysis_entiredataset_SCCoreGEP_vs_BWH.csv")
sccoregep_ajcc8_fn <- file.path(results_dir, "intermediate", "validation", "d_squame", "multivariable_analysis_entiredataset_SCCoreGEP_vs_AJCC8.csv")
sccoregep_emcmodel_fn <- file.path(results_dir, "intermediate", "validation", "d_squame", "multivariable_analysis_entiredataset_SCCoreGEP_vs_EMCmodel.csv")
# Output folder
...
fp_df <- df %>%
    ...
    mutate(analysis_term = factor(analysis_term,
                                  levels = c("SCCore-GEP vs AJCC8:T2-T3", "SCCore-GEP vs AJCC8:SCCore-GEP", "SCCore-GEP vs AJCC8",
                                             "SCCore-GEP vs BWH:T2a-T2b", "SCCore-GEP vs BWH:SCCore-GEP", "SCCore-GEP vs BWH",
...
# Save results
#-------------------------------------------------------------------------------
write.csv(source_data_df, file.path(output_dir, "supp_figure_13b_entiredataset_multivariable_analysis_source_data.csv"), row.names = F)
pdf(file.path(output_dir, "supp_figure_13b_entiredataset_multivariable_analysis.pdf"), height = 2.5, width = 16)
print(fp)
dev.off()
#-------------------------------------------------------------------------------