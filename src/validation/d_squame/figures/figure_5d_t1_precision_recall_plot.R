#-------------------------------------------------------------------------------
# Aim: Plot precision-recall plot for T1 in D-SQUAME validation
# Author: L.Pozza
# Input: SCCore-GEP threshold-based metrics in T1
# Output: Figure 5D: T1 precision recall plot
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
t1_res <- res %>%
       filter(stratification_system == "BWH_staging" & stratum == "T1") %>%
...
stages_risk_bwh <- data.frame(Stratum = c("T1"),
                              risk = c(0.76),
                              prevalence = c(0.70))
regions_of_interest <- t1_res %>%
       filter(`Post-test risk` > 1.9) %>%
       head(1) %>%
...
# Make and save plot
#-------------------------------------------------------------------------------
ppv_vs_se <- ggplot(data = t1_res %>% arrange(Threshold),
                          aes(x = Sensitivity,
                              y = `Post-test risk`)) +
...
perc_hr_vs_se <- ggplot(data = t1_res %>%
                              group_by(`% patients high-risk`, Sensitivity) %>%
                              unique() %>%
                              arrange(`% patients high-risk`),
                          aes(x = Sensitivity,
                              y = `% patients high-risk`)) +
                        geom_ribbon(aes(ymin = 0, ymax = `% patients high-risk`, fill = "% patients high-risk"), alpha = 0.6)+
                        geom_ribbon(aes(ymax = 100, ymin = `% patients high-risk`, fill = "% patients low-risk"), alpha = 0.6)+
                        geom_vline(data = regions_of_interest, aes(xintercept = Sensitivity), linetype = 2, linewidth = 0.4)+
                        scale_fill_manual(values = c("% patients high-risk" = "#CC79A7", "% patients low-risk" = "#009E73"))+
                        theme_bw() +
...
#-------------------------------------------------------------------------------
source_data_df <- t1_res %>% select(-Threshold)
#-------------------------------------------------------------------------------

# Save results
#-------------------------------------------------------------------------------
write.csv(source_data_df, file.path(output_dir, "figure_5d_t1_precision_recall_plot_source_data.csv"), row.names = F)
pdf(file.path(output_dir, "figure_5d_t1_precision_recall_plot.pdf"), height = 5, width = 3.5)
print(p)
dev.off()
#-------------------------------------------------------------------------------